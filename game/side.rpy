...
label side_Gun_1:
    ...
    dt "Zarabia nasze drogocenne eurodolce"
    dt "A wydaje je w jakimś kraju, takim innym nie u nas"
    dt "Dlatego, jak tylko pojawi się w Night City to zjem go"
    dt "To rozwiązanie polecił mi mój drogi przyjaciel, Enrico Bobrico"
    dt "On jest dobrym polakiem, jego nie zjem"
    dt "Ale ten okropny Joe, zapewne smakuje okropnie też"
    dt "Jestem gotowy na to poświęcene zjedzenia go"
    dt "Nikt inny w tym cudownym mieście nie powinien tak cierpieć"
    dt "Joe Gajden okrada amerykańskie dzieci"
    dt "Wiem bo czesto mam kontakt z dziećmi"
    dt "Mówią mi, Panie Trump ten Joe Gajden nas okrada"
    dt "Ja im mówie że na to nie pozwole"
    dt "I uwierzcie mi że jak czegoś nie pozwole to wtedy to sie nie dzieje, jestem w tym najlepszy"
    dt "Mój doradca sprawdził stan jego konta"
    dt "I wymyślił na niego ksywkę Bartosz Biedak"
    dt "A to dlatego, że nałożyłem na niego taryfy"
    dt "I już nie zarabia tak ładnie"
    dt "Ogólnie Polska to jest piękny kontynent ale ten Joe"
    dt "Świat będzie cichszy bez niego, a ja mam problem ze słuchem"
    dt "On jest odpowiedzialny za zagłuszanie naszej komunikacji"
    dt "Jak wysłałem mu o tym wiadomość na Potężnym Patriocie"
    dt "To mi odpisał, że jego matula jest głośniejsza"
    dt "Szczyt chamstwa, meksykańcom zabierałem za to wizy"
    dt "A on nie ma jeszcze, ale pewnie będzie chciał"
    dt "Bo w tym kraju każdy chce być"
    p "Mam już dość jego pierdolenia"
    g "No dobra, tyle nam wystarczy ale przyjedziemy na następny wiec"
    p "No dobra, to wracamy?"
    g "Wracamy"
